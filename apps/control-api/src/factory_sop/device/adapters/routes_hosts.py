    ...
    match requested.status:
        case DeviceStatus.DEACTIVATED:
            host = deactivate_host(
                host_id=host_id, caller=caller, now=datetime.now(UTC), hosts=hosts
            )
        case DeviceStatus.ACTIVE:
            host = restore_host(host_id=host_id, caller=caller, now=datetime.now(UTC), hosts=hosts)
    return _view(host)
...
@router.delete(
    ...
    | {
        404: problem_openapi_response("Host not found"),
        409: problem_openapi_response("Host still carries backends"),
    },
)
def delete_a_host(
    host_id: UUID,
    ...
    delete_host(host_id=host_id, caller=caller, hosts=hosts, backends=backends)
