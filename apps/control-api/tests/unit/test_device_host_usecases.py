...
def test_an_unrelated_permission_does_not_grant_host_edits() -> None:
    hosts = FakeInferenceHosts()
    ...
    # Holding another module's edit says nothing about this one: the permission set is flat.
...
@pytest.mark.parametrize(
    ("recording_window_seconds", "disk_watermark_percent", "field"),
    [
        (0, 85, "recording_window_seconds"),
        (7 * 24 * 3600, 0, "disk_watermark_percent"),
        (7 * 24 * 3600, 100, "disk_watermark_percent"),
    ],
)
def test_edit_host_rejects_invalid_recording_configuration_at_the_usecase_seam(
    recording_window_seconds: int, disk_watermark_percent: int, field: str
) -> None:
    hosts = FakeInferenceHosts()
    host = hosts.register(name="装配A线-推理机1")
...
def test_deactivating_a_host_is_reversible_and_a_replay_changes_nothing() -> None:
    ...
    deactivated = deactivate_host(
        host_id=host.id, expected_revision=host.revision, caller=CALLER, now=LATER, hosts=hosts
    )
    replayed = deactivate_host(
        host_id=host.id,
...
def test_status_changes_refuse_a_stale_revision_before_a_noop_or_write() -> None:
    hosts = FakeInferenceHosts()
    host = hosts.register(name="装配A线-推理机1")
...
def test_delete_refuses_a_stale_revision_before_removing_an_empty_host() -> None:
    hosts = FakeInferenceHosts()
    host = hosts.register(name="装配A线-推理机1")
