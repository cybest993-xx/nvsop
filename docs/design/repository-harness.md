...
The machine-readable list of center modules is `[tool.nvsop]` in the root `pyproject.toml`. The migration-ownership and repository-policy gates read that table directly; this table and the roadmap's §六 explain it. Adding or removing a module starts there — a module named in prose but not in the declaration owns no tables and passes no gate.
...
| `configuration` | no tables; composes an inference-host configuration bundle through read seams                                                                                                      | pull host configuration                                              | device/template read seams                    |
...
| `overview`      | no tables; composes permission-scoped owner summaries                                                                                                                              | read overview                                                        | HTTP composition adapter                      |
...
Required: preserve authorization, data integrity, architecture invariants, necessary verification and resolution of blocking findings. Use one `agent/<agent-id>/<task-slug>` branch and one main session responsible for completion. Read the [local branch workflow](local-branch-workflow.md) before changing branches, worktrees or refs. Apply [§4](#4-test-placement-and-evidence) for testing, evidence reuse, review and stopping decisions.

**Workspace isolation.** Start each task from the accepted `main` tip in its own worker worktree; `main` is the shared trunk and is not a task workspace, and `dev` is retired from the delivery path. Use an isolated worktree outside the repository when the checkout has unrelated work, multiple writers run in parallel, a shared development instance uses the checkout, or switching branches would disrupt another task or running service. Publish the completed `agent/...` branch and open its pull request directly against `main`; after the remote merge, synchronize local `main` only from the fetched remote-tracking `main`. A larger, cross-module or long-lived task should use isolation when its impact on other work cannot be bounded. Inspect status before edits; do not discard, auto-stash or overwrite unrelated changes. Continue an existing task on its existing branch/worktree rather than creating a new one for each window or review repair.
...
The [local branch workflow](local-branch-workflow.md) is the authoritative branch-policy document. Its `reference-transaction` hook, versioned in `scripts/githooks/` and enabled by `make hooks`, checks the actual local ref transaction: local `main` may synchronize only to a fetched remote-tracking `main`, `dev` creation or advancement is rejected while deletion of a legacy ref is allowed, and new task branches must use the worker namespace. `pre-commit` provides early feedback, while `pre-push` rejects direct pushes to remote `main`; vendor and Python-format checks remain separate local guardrails. These hooks do not isolate worktrees or prove server-side enforcement; removing or replacing local hook configuration is outside a repository-owned hook's control.
