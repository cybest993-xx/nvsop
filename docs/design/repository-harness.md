...
**Workspace isolation.** Start each task from the current `dev` tip in its own worker worktree; `main` and `dev` are shared refs, not task workspaces. Use an isolated worktree outside the repository when the checkout has unrelated work, multiple writers run in parallel, a shared development instance uses the checkout, or switching branches would disrupt another task or running service. The integration operator owns a separate `dev` worktree and serializes merges into `dev` before promoting it to `main`. A larger, cross-module or long-lived task should use isolation when its impact on other work cannot be bounded. Inspect status before edits; do not discard, auto-stash or overwrite unrelated changes. Continue an existing task on its existing branch/worktree rather than creating a new one for each window or review repair.
...
The [local branch workflow](local-branch-workflow.md) is the authoritative branch-policy document. Its `reference-transaction` hook, versioned in `scripts/githooks/` and enabled by `make hooks`, checks the actual local ref transaction: `main` and `dev` lifecycle and source rules, worker branch names, and the explicit remote-tracking-`main` reset exception. `pre-commit` provides early feedback, while the existing `pre-push`, vendor and Python-format checks remain separate local guardrails. These hooks do not isolate worktrees or prove server-side enforcement; removing or replacing local hook configuration is outside a repository-owned hook's control.
