...
关键在于基座原本没有把服务路径回调可观测的 `INVALID`（source error）、`PLAYING`（delivering）与 EOS 归一成 SSE 流健康事实；源元件内部的重连没有独立总线消息。S010 进一步实测：queue FIFO 本身不够——旧 frame 可已进入 chunking 但尚未产出动作，DDM metadata callback 可在 drain 后晚到，response loop 可阻塞在旧 VLM future，EOS 也可先于有效尾块。登记补丁因此在真实 owner seam 用 stream epoch / producer tag / wake Event / deferred EOS 收敛这些竞态；真实 PTS 回退仍更新 `first_timestamp`，下一正常 chunk 才携带新锚点。
