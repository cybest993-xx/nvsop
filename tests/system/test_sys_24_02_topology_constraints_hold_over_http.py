...
def test_topology_constraints_hold_over_http(client: Client, bootstrap: BootstrapCommand) -> None:
    ...
    deactivated = client.put(
        f"{HOSTS}/{host_id}/status", json={"status": "deactivated"}, headers=headers
    )
    ...
    restored = client.put(f"{HOSTS}/{host_id}/status", json={"status": "active"}, headers=headers)
    assert restored.status_code == 200
    assert client.delete(f"{HOSTS}/{host_id}", headers=headers).status_code == 409
    assert client.delete(f"{BACKENDS}/{backend_id}", headers=headers).status_code == 204
    assert client.delete(f"{HOSTS}/{host_id}", headers=headers).status_code == 204
