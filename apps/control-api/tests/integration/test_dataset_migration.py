"""用真实 PostgreSQL 走过 0020 → 0021 的训练数据集迁移。"""
...
@pytest.fixture
def database_at_0020(engine: Engine) -> Iterator[Engine]:
    """在独立数据库中保留 0020 状态，再由测试执行升级。"""
    ...
    command.upgrade(configuration, "0020")
    try:
        yield upgraded
    finally:
        upgraded.dispose()
...
def test_training_dataset_migration_upgrades_and_rolls_back_on_real_postgres(
    database_at_0020: Engine,
) -> None:
    assert "job_application_job" in _tables(database_at_0020)
    ...
    configuration.set_main_option("script_location", str(CONTROL_API / "migrations"))
    configuration.set_main_option(
        "sqlalchemy.url", database_at_0020.url.render_as_string(hide_password=False)
    )
    ...
    assert {
        ...
        "failure_code",
        "recovery_action",
    } <= _columns(database_at_0020, "dataset_member")
    assert {
        "id",
        "dataset_id",
        "member_id",
        ...
        "validation_job_id",
        "object_version_id",
    } <= _columns(database_at_0020, "dataset_upload_attempt")

    with database_at_0020.connect() as connection:
