...
Read Git history, final CI and live Issue state before recording completion. Record commands/results, evidence location and remaining gaps. When Issue closure is a planned stage under [workflow.md](workflow.md), comment the outcome, close via `gh issue close <number> --comment <summary>`, and add the result pointer to the map's Decisions-so-far where applicable. Do not pause for another confirmation once closure evidence is complete. Stop once acceptance is met; optional refactors and hardening need a separate demonstrated requirement.
