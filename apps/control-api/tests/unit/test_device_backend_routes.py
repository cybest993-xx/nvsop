...
def test_a_backend_needs_an_existing_active_host(center: Center) -> None:
    ...
    assert (
        center.send("PUT", f"{HOSTS}/{host_id}/status", json={"status": "deactivated"}).status_code
        == 200
    )
    refused = center.send(
...
def test_a_connection_test_reports_success_and_failure_from_the_probe(center: Center) -> None:
    ...
    reached = center.send("POST", f"{BACKENDS}/{backend_id}/connection-test")
    ...
    failed = center.send("POST", f"{BACKENDS}/{backend_id}/connection-test")
...
def test_the_backend_lifecycle_is_reversible_and_deletable(center: Center) -> None:
    ...
    deactivated = center.send(
        "PUT", f"{BACKENDS}/{backend_id}/status", json={"status": "deactivated"}
    )
    assert deactivated.status_code == 200
    assert deactivated.json()["status"] == "deactivated"
    restored = center.send("PUT", f"{BACKENDS}/{backend_id}/status", json={"status": "active"})
    assert restored.status_code == 200
    assert restored.json()["status"] == "active"

    assert center.send("DELETE", f"{BACKENDS}/{backend_id}").status_code == 204
...
def test_a_caller_without_backend_edit_permission_is_refused_with_403(center: Center) -> None:
    ...
    denied = center.send("POST", f"{BACKENDS}/{backend_id}/connection-test")
...
def test_deleting_a_host_is_possible_once_its_backends_are_gone(center: Center) -> None:
    ...
    assert center.send("DELETE", f"{HOSTS}/{host_id}").status_code == 409
    assert center.send("DELETE", f"{BACKENDS}/{backend_id}").status_code == 204
    assert center.send("DELETE", f"{HOSTS}/{host_id}").status_code == 204
