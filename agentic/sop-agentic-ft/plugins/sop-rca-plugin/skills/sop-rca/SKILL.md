...
| `chunking_algorithm`, `chunk_length_sec` | E2E driver log `sop_e2e_eval_log.txt` (top-of-file `Args: Namespace(...)` line). Tells you whether segmentation was DDM-driven or uniform-time. | parse log | Step 5a (skip DDM analysis when `chunking_algorithm=uniform`); when uniform, check whether the `wmcq` augmentation stage is enabled — see Pattern 1 fix 4 |
