...
Nginx 会为已授权的标注派生媒体承担一次低频字节转发，但不会改变运行态视频的直连边界，也不会让源视频上传经过网关。`docs/deployment/nginx-annotation.conf.example`、[仓库架构](../engineering/architecture.md) 和控制面机制文档共同引用本 ADR，避免把该例外误读为全局视频中继许可。
