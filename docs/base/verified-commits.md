...
- `0001-stream-health-events.patch` 仍只触及 `ds_sop_process.py` 且零删除；逻辑上是模块 import + `on_message` 健康 hook + internal-vLLM 旧帧队列清理 + uniform/DDM 两条时间轴处理，unified diff 机械上形成 6 个连续追加 run。
