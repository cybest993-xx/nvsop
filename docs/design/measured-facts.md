...
关键在于基座原本没有把服务路径回调可观测的 `INVALID`（source error）、`PLAYING`（delivering）与 EOS 归一成 SSE 流健康事实；源元件内部的重连没有独立总线消息，恢复后的时间轴变化只能通过 `first_timestamp` 比较得到。`checker_result.error_message` 也只反映 checker 自身异常。
