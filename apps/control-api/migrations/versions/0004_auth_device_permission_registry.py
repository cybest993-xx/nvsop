...
# The host members `device` registers in `auth/permissions.py` with this same change, in enum
# order. Backend permissions are deliberately left for the later backend-management migration;
# a role cannot grant a permission that has no row here, so the rows and enum members stay one
# phase together.
DEVICE_PERMISSIONS = (
