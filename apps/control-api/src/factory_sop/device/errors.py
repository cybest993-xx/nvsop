...
class DeviceRefusalCode(StrEnum):
    """The `error_code` values `device` produces. `STALE_REVISION` is §5.15's shared one."""
...
def refusal_problem(code: DeviceRefusalCode) -> tuple[int, str]:
    ...
    Beside the codes it maps, because a new code and its wire meaning are one change. The
    title is what a client that does not recognize the `error_code` displays verbatim
    (§5.15's unknown-value fallback).
    """
    match code:
