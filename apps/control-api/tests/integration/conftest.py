...
Center training media uses a real local persistent directory owned by the suite, not an
external object store: the deployment has no object-storage service to bring up.
"""
...
import pytest
from _integration_support import RedisServer
...
@pytest.fixture(scope="session")
def dataset_storage_root(tmp_path_factory: pytest.TempPathFactory) -> Path:
    """训练素材使用真实中心本地持久目录，而不是外部对象存储。"""
    return tmp_path_factory.mktemp("dataset-files")
