...
`LocalConfigurationStore` 只持久化**实际已经生效**的 bundle。候选配置先完成 host scope、revision/同 revision 冲突、能力要求和完整运行组合校验；这些步骤不会前移 durable confirmed。运行中的候选只有在组合对象已成功构造后才请求旧循环停机，随后先切换实际 runtime，再由 `LocalConfigurationStore` 在 SQLite 单事务确认同一候选。组合或应用失败继续旧配置并记录 `local_config_failure`；本地确认失败会先撤回候选内存指针并关闭候选工位资源，不启动新 runtime 循环，随后失败退出并从最后 durable confirmed 恢复。启动时同样先组合候选、成功后才确认；首次无确认值且中心不可达时才使用本地 bootstrap。
