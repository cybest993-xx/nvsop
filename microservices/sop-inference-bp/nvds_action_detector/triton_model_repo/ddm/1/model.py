...
class DDMTensorRTEngine:
    ...
    def infer(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        # execute_async_v3 can also enqueue work on TRT *auxiliary* streams beyond trt_stream,
        # so syncing only trt_stream leaves TRT GPU work in flight and the downstream
        # DeepStream gst-CV NvBufSurfTransform/cudaMemcpy2DAsync races freed/in-use surface
        # memory -> driver-layer SIGSEGV (surfaces on the cached-engine path). A full-device
        # sync drains ALL TRT work (incl. aux streams) before returning.
