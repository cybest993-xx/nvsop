...
- **C2** 鉴权 + 自定义权限 + 诊断日志基线（`auth`；诊断日志按其自身标准建设，不为审计要求预留形状，Q37）。**诊断日志基线已随 C1 落地**（`factory_sop/observability/`，见 §5.15）。🔄 C2.1 已完成（`factory_sop.auth`：本地账户 Argon2 口令、服务端会话记录、空闲与绝对两档超时、首账户 bootstrap 命令 `python -m factory_sop.bootstrap`——无账户的全新部署由此可达登录；「停用即撤销全部会话」的**会话侧规则** `revoke_every_session_of` 已落地，其调用方——停用用例——随 C2.2；行为经 `repository.py` 的 `Protocol` 与存储解耦，`problem+json` 与 CSRF 双提交在 `adapters/`；含 `auth` 诊断行「五必填字段且不含任何形式凭据」的整键集回归。验收场景按 §5.15 以 SYS-22-01…07 随本切片给出，`tests/system/` 以同 ID 命名（SYS-22-07 为 web 布局基线，由 control-web 套件承载），随 `make check-integration` 的 `center-system` 运行。**pnpm workspace 提前在此落地**——本切片交付中文登录页与受保护布局，故 C7 的「pnpm workspace 在此落地」已由本项承担；`make check` 同一变更内扩出 web 的 install/format/lint/type/unit/build 六目标，锁文件与 Node 运行时之钉由 `check_repo_policy.py` 在 `apps/control-web/` 存在后强制。）🔄 **C2.2 功能实现已完成，待独立合并门禁通过后关闭**（用户、角色与自定义权限管理：权限为固定三段式枚举 `module.resource.action`、粒度 view/edit/delete（§5.15），登记于代码枚举 + `auth_permission` 表（外键拒绝未注册值，新模块的权限随该模块变更以 auth 前缀迁移登记）；角色是权限集合的普通行——bootstrap 以普通角色「系统管理员」授予全部注册权限，可改名收窄删除，未设内置不可变角色；授权决策 `authorize` 是纯函数、调用点在 `usecases/` 层，`Caller` 由 HTTP 适配层一次解析为值传入，脚本/ARQ worker 同样绕不过；「最近管理员」守卫拒绝会清空 `auth.user.edit`+`auth.role.edit` 在用持有者的操作（改角色、删角色、改赋角色、停用、删除五条路径同一个函数）；写路由在 OpenAPI 以 `x-required-permission` 声明权限，`test_authorization_is_enforced` 按行为机械校验「每个写路由声明且与用例实际检查一致」（路由覆盖完备性 + 无权限必拒 + 有权限必过）；停用/删除在同一事务内撤销全部会话（AC4）；变更归属落 `created_by`/`updated_by` 列，实体变更与拒绝均有稳定诊断事件（`auth.user.created` 等、`auth.authorization.refused`）；web 新增「用户与权限」模块（账户/角色两页签，只渲染调用方有权使用的操作，权限清单由后端 `/auth/permissions` 供给），导航项按权限显隐；`SessionView` 增 `permissions` 有效权限集——按 ADR-0003 兼容门在 schema 中标记为可选（新必需字段被拒），载荷始终携带；验收场景 SYS-23-01…04 随 `tests/system/` 落地，随 `center-system` 运行。改动按 harness §5 的变更预算审查，并拆解为四个可独立合并阶段：**C2.2a 授权词汇与授权基座**（权限枚举、`Caller`、bootstrap 归属）；**C2.2b 角色管理**（角色表、角色 CRUD、最后管理员守卫）；**C2.2c 用户管理**（账户 CRUD、角色分配、停用/恢复与会话撤销）；**C2.2d Web 与契约**（用户与权限页、生成客户端及 OpenAPI/浏览器契约）。后一个阶段只依赖前一个已合并的接口，不在同一变更中用提交拆分伪造预算，也不为行数创建新的产品模块或透传接口；每阶段必须带自己的行为测试并留下可运行的中间状态。）
