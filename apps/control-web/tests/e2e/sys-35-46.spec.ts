...
test('SYS-35-46 — SSE mirrors pass, fail, and indeterminate verdicts', async ({ page }) => {
  ...
  await expect(page.getByText('结论 pass')).toBeVisible()
  await expect(page.getByText('结论 fail')).toBeVisible()
  await expect(page.getByText('结论 indeterminate')).toBeVisible()
