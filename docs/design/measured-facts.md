...
关键在于基座原本没有把服务路径回调可观测的 `INVALID`（source error）、`PLAYING`（delivering）与 EOS 归一成 SSE 流健康事实；源元件内部的重连没有独立总线消息。S010 进一步实测：queue FIFO 本身不够，健康迁移可能发生在旧 frame 已进入 chunking、但动作尚未入队时；PTS 回退也可能留下旧 descriptor 等待新时间轴。因此登记补丁用 stream epoch 在 owner seam 退休尚未完成的旧 normalization work，并在真实 PTS 回退时更新 `first_timestamp`；下一正常 chunk 才携带新锚点。
